...
```{r demo-ari-ami, fig.width=6, fig.height=4, echo=FALSE}
# Attach SNITCH here too (knit happens in a fresh R session)
library(SNITCH)
# Guarded loads so README knit doesn't break if optional deps are missing
need <- c("dbscan","Mfuzz","Biobase","factoextra","ggrepel","aricode","tibble","dplyr","ggplot2")
ok <- vapply(need, requireNamespace, quietly = TRUE, FUN.VALUE = logical(1))
if (!all(ok)) {
  msg <- paste("Skipping ARI/AMI demo (missing packages):", paste(need[!ok], collapse = ", "))
  plot.new(); title(msg)
} else {
  library(dbscan); library(Mfuzz); library(Biobase)
  library(factoextra); library(ggrepel); library(aricode)
  library(dplyr); library(tibble); library(ggplot2)

  scaled <- prepare_data(data = t(methylation_df), age = ages_df$Age)
  cls <- run_parallel_classification(dat_scaled = scaled$dat_scaled, age = scaled$Age, ages_grid = scaled$ages_grid)

  nl_rows <- cls[grep("NL", cls$classification), ]
  if (nrow(nl_rows) < 2) {  # ensure enough NL sites for FPCA/clustering
    plot.new(); title("Not enough NL CpGs in this tiny demo to run clustering."); 
  } else {
    nl_smooth <- do.call(rbind, nl_rows$Predictions)
    rownames(nl_smooth) <- nl_rows$CpG

    fpca <- perform_fpca(nl_var_smooth = nl_smooth, ages_grid = scaled$ages_grid)

    truth <- groups_df$Group
    class_wrk <- cls; nl_mask <- class_wrk$classification == "NL"
    add_metrics <- function(pred, label) tibble(Method = label, ARI = aricode::ARI(pred, truth), AMI = aricode::AMI(pred, truth))
    df_comp <- tibble(Method=character(), ARI=double(), AMI=double())

    # MFUZZ
    eset <- new("ExpressionSet", exprs = fpca$scores)
    m_opt <- mestimate(eset); c_opt <- min(6, nrow(fpca$scores))
    mf <- mfuzz(eset, c = c_opt, m = m_opt)
    pred_fuzzy <- paste0("NL_", apply(mf$membership, 1, which.max))
    cw <- class_wrk; cw$classification[nl_mask] <- pred_fuzzy
    df_comp <- dplyr::bind_rows(df_comp, add_metrics(cw$classification, "SNITCH + Fuzzy"))

    # k-means
    k_opt <- min(6, nrow(fpca$scores))
    km <- kmeans(as.matrix(fpca$scores), centers = k_opt, nstart = 20)
    pred_km <- paste0("NL_", km$cluster)
    cw <- class_wrk; cw$classification[nl_mask] <- pred_km
    df_comp <- dplyr::bind_rows(df_comp, add_metrics(cw$classification, "SNITCH + K-Means"))

    # HDBSCAN
    hdb <- hdbscan(as.matrix(fpca$scores), minPts = 5)
    pred_hdb <- paste0("NL_", hdb$cluster)
    cw <- class_wrk; cw$classification[nl_mask] <- pred_hdb
    df_comp <- dplyr::bind_rows(df_comp, add_metrics(cw$classification, "SNITCH + HDBSCAN"))

    p_comp <- ggplot(df_comp, aes(ARI, AMI, label = Method)) +
      geom_point(size = 3) +
      geom_text_repel(size = 3) +
      coord_equal() +
      labs(title = "Clustering agreement on simulated data",
           x = "Adjusted Rand Index (ARI)", y = "Adjusted Mutual Information (AMI)") +
      theme_minimal()
